...
Pipeline steps:
    1. ensure_permissions()
    2. find_wechat_window(config.wechat_app_name)
    3. list_unread_chats(window) -> filter by groups_to_monitor
    4. For each unread chat:
       a. click_into_chat(chat)
       b. scroll_chat_to_bottom(window)
       c. read_messages_from_uitree(window, chat.name)
...
def run_pipeline_a(config: WeclawConfig) -> list[str]:
    ...
    unread_chats = list_unread_chats(window)
    ...
    for chat in target_chats:
        ...
        messages = read_messages_from_uitree(window, chat.name)
        if messages:
