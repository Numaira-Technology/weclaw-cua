...
Input spec:
    - config: WeclawConfig with report_custom_prompt, llm_model, llm_provider, llm_api_key.
...
Pipeline steps:
    ...
    3. generate_report(prompt, config.llm_model, config.llm_api_key, config.llm_provider)
"""
...
def run_pipeline_b(config: WeclawConfig, message_json_paths: list[str]) -> str:
    ...
    report = generate_report(prompt, config.llm_model, config.llm_api_key, config.llm_provider)
    return report
