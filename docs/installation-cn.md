...
| 微信桌面版 | 任意版本 |
| LLM 接入 | OpenClaw gateway（推荐）或所选 provider 的 API key |
...
`llm_provider` 可设为 `openrouter`、`openai`、`deepseek`、`kimi`、`glm` 或 `qwen`。只有在使用内置 LLM 模式时才需要填写对应 API key；当 `llm_provider` 为 `openrouter` 时，所有模型都会通过 OpenRouter 调用。如果通过 OpenClaw gateway 或 stepwise 模式运行，请保持为空。
...
| `llm_model` | LLM 模型 ID。OpenRouter 会原样发送完整 slug；原生 provider 会去掉 `provider/` 前缀后调用。 |
...
export KIMI_API_KEY="sk-你的-kimi-key"
export GLM_API_KEY="sk-你的-glm-key"
export QWEN_API_KEY="sk-你的-qwen-key"
...
$env:KIMI_API_KEY = "sk-你的-kimi-key"
$env:GLM_API_KEY = "sk-你的-glm-key"
$env:QWEN_API_KEY = "sk-你的-qwen-key"
weclaw-cua run          # 捕获 + 生成报告
weclaw-cua capture      # 仅捕获
weclaw-cua report       # 从已有捕获生成报告
```

也可以直接在 `config/config.json` 的对应 `*_api_key` 字段中填入 key。
